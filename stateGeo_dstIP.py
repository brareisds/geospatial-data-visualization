...
# Definir os intervalos de valores e as cores correspondentes para fazer uma comparacao estatica das ocorrencias 
...
for i, month in enumerate(df_state_brazil['Month'].unique(), start=1):
    ...
    directory = 'gpd-maps-images/dstIPs/brazil-states/'
    # plt.savefig(os.path.join(directory, f"Figure_{i}.png"))
    plt.savefig(os.path.join(directory, f'state_dstIPs_{month}.png'), dpi=150)
...
for month in sorted_month_year_list:
    images.append(imageio.imread(f'{directory}state_dstIPs_{month}.png'))
...
imageio.mimsave(os.path.join(directory, 'dstIPs_states.gif'), images, fps=1)
